...
// Get the most recent unread notifications for a seller—used for dropdown
function getSellerUnreadNotifications($sellerId, $limit = 10) {
    global $pdo;
    try {
        $stmt = $pdo->prepare("
            SELECT * FROM seller_notifications
        ...
        error_log("Error getting unread seller notifications: " . $e->getMessage());
        return [];
    }
}
