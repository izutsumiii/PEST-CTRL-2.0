...
<head>
    ...
    <style>
        * {
        ...
        /* Normalize seller header/sidebar sizing and fonts */
        .invisible-header { height: 60px; display:flex; align-items:center; justify-content:space-between; padding: 0 12px; background:#130325; }
        .header-right, .header-left { height: 60px; display:flex; align-items:center; }
        .header-user .header-user-info span { font-size: 14px; font-weight: 700; color: #F9F9F9; }
        .seller-notifications .notification-dropdown { width: 360px; }
        .seller-notifications .notification-item { font-size: 13px; }
        .seller-notifications .notification-title { font-size: 13px; font-weight: 700; }
        .seller-notifications .notification-message { font-size: 12px; }
        .seller-notifications .notification-time { font-size: 11px; }
        ...
        /* Override for Clear All button - must be red */
        .notification-header button[onclick*="clearAllSellerNotifications"] {
    ...
    <div class="invisible-header">
        ...
        <div class="header-right">
            ...
            <div class="header-user">
                <?php
                // Resolve a proper display name for the signed-in user (prefer FIRST NAME for seller)
                $displayName = '';
                if (!isset($pdo) || !$pdo) { @include_once __DIR__ . '/../config/database.php'; }
                $dbUsername = null; $dbFirst = null; $dbLast = null; $dbDisplay = null;
                if (isset($pdo) && $pdo) {
                    try {
                        $userId = isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : 0;
                        if ($userId > 0) {
                            $stmt = $pdo->prepare("SELECT username, first_name, last_name, display_name FROM users WHERE id = ? LIMIT 1");
                            $stmt->execute([$userId]);
                        } else {
                        ...
                        $dbFirst = $row['first_name'] ?? null;
                        $dbLast = $row['last_name'] ?? null;
                        $dbDisplay = $row['display_name'] ?? null;
                        // Per request: use first name primarily for header
                        if (!empty($dbFirst)) {
                            $displayName = trim($dbFirst);
                        } elseif (!empty($dbDisplay)) {
                            $displayName = trim($dbDisplay);
                        } elseif (!empty($dbUsername)) {
                            $displayName = trim($dbUsername);
                        }
                    } catch (Exception $e) {
                        $displayName = isset($_SESSION['username']) ? $_SESSION['username'] : 'User';
                    }
                }
                if ($displayName === '' && isset($_SESSION['username'])) { $displayName = $_SESSION['username']; }
                $avatarInitial = strtoupper(substr((string)$displayName, 0, 1));
                
                ?>
                <div class="header-user-info" onclick="toggleHeaderDropdown()">
                    <div class="header-user-avatar"><?php echo $avatarInitial ?: 'U'; ?></div>
